...
void iniEquilibrium(Node* node, float rho, float ux, float uy) {
    int iPop;
...
void saveF(Simulation* sim, int iPop, char fName[]) {
    ...
    for (iY=1; iY<=sim->ly; ++iY) {
       for (iX=1; iX<=sim->lx; ++iX) {
           float f = sim->lattice[iX][iY].fPop[iPop];
           fprintf(oFile, "%f ", f);
       }
       fprintf(oFile, "\n");
    }
...
void computeMacros(float* f, float* rho, float* ux, float* uy) {
    float upperLine  = f[2] + f[5] + f[6];
    float mediumLine = f[0] + f[1] + f[3];
    float lowerLine  = f[4] + f[7] + f[8];
...
float computeEquilibrium(int iPop, float rho,
                          float ux, float uy, float uSqr)
{
    float c_u = c[iPop][0]*ux + c[iPop][1]*uy;
    return rho * t[iPop] * (
  ...
  // bgk collision term
void bgk(float* fPop, void* selfData) {
    float omega = *((float*)selfData);
    float rho, ux, uy;
